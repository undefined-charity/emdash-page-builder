...
- **A ribbon** (Word's Home tab): undo/redo, paragraph styles, font, size, bold/italic/underline/strike, text colour, highlight, alignment, lists, quotes, links, insert. Its commands come in groups that stay together; in a narrow window whole groups move to another row, and the page and side panel make room.
...
- **Deleting** a block that holds a lot (a section with several blocks, columns, cards, a gallery, a long text) asks first; undo works either way.
- **Drag handles** on every block, including blocks inside sections and columns. A block inside a section, column or card keeps its handle within that container, and the handle stays with its block while you move the pointer to it.
- **Adding blocks anywhere**: the handle's **+** adds a block below; every section, column, card, FAQ answer and layer, and the page itself, ends in a faint **+ Add block** row; and the side panel has **＋ Before**, **＋ Inside** and **＋ After** for the selected block. As a block is dragged, the page opens a gap where it will land. Drop it at the left or right edge of another block to put the two side by side in columns. <kbd>Esc</kbd> cancels.
...
- **Pages** (📄 in the ribbon): the site's pages. A page goes into a menu only once it's published (a link to an unpublished page would lead nowhere), and trashing or unpublishing it removes its menu links. Open one to edit it, add one (blank, or a copy of another) and it opens ready to edit, change a page's title, address and search and sharing description (and share image, where the collection has EmDash's SEO settings), tick the menus it appears in, unpublish it, or move it to the trash. Another page's changes are published along with the page being edited; an address change takes effect when the page is published. Pages the site routes itself (home, 404…) can't be renamed or removed here.
- **Links to pages**: typing in the ribbon's link box suggests the site's pages.
- **History** (🕘 in the ribbon): every saved version of what you're editing (the page, or the header or footer), with when and who, quick saves grouped into editing sessions. See any version as visitors would, side by side with the current draft if you like, and restore it as the draft (publishing is still up to you). Name and star versions to find them again; versions that went live are marked, and **Only named and published** filters to them.
